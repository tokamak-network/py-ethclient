...
또한 **Groth16 ZK 증명 툴킷**을 내장하고 있습니다 — circuit 정의, trusted setup, proof 생성, 네이티브 검증, EVM on-chain 검증을 단일 프로세스에서 실행할 수 있는 유일한 Python 환경입니다. AI 코딩 에이전트와 함께 ZK circuit을 프로토타이핑하고 디버깅하는 가장 빠른 방법입니다.

또한 **L1↔L2 General State Bridge**를 내장하고 있습니다 — Optimism 스타일의 CrossDomainMessenger로 L1과 L2 간 임의 메시지를 릴레이하며, 타겟 도메인의 EVM에서 실제 상태 변경을 실행합니다. 검열 저항을 위한 force inclusion과 가치 복구를 위한 escape hatch를 포함합니다.

RLP 인코딩, 머클 패트리시아 트라이, EVM 실행, RLPx 전송 암호화, eth/68·snap/1 와이어 프로토콜, Discovery v4, Engine API, Groth16 ZK 증명, L1↔L2 브릿지 등 모든 핵심 프로토콜 로직을 순수 Python으로 직접 구현했습니다. 외부 의존성은 암호화 프리미티브와 웹 프레임워크만 사용합니다.
...
- **L1↔L2 General State Bridge** — Optimism 스타일 CrossDomainMessenger로 플러거블 릴레이 핸들러 (EVM, Merkle proof, ZK proof, TinyDB, direct state), 임의 크로스 도메인 메시징, force inclusion (검열 저항), escape hatch (가치 복구)
...
- **JSON-RPC 2.0** — `eth_call`, `eth_estimateGas`, 트랜잭션/영수증 조회, 로그 쿼리, `zk_` 네임스페이스 등 20개 이상 메서드
...
- **713개 테스트** — RLP부터 ZK 증명, L2 브릿지, 통합 테스트까지 전 프로토콜 레이어를 커버하는 포괄적 테스트 스위트
- **Docker 지원** — Docker Compose로 간편 배포
- **L2 확장성** — 실행 훅 시스템, 플러거블 릴레이 핸들러 (EVM/Merkle/ZK/TinyDB/Direct), CrossDomainMessenger 브릿지로 EVM 코어 수정 없이 Layer 2 개발 가능
...
- **L2 개발** — 내장된 실행 훅 시스템과 L1↔L2 브릿지를 통해 크로스 도메인 메시징부터 force inclusion, escape hatch까지 — EVM 코어 코드를 수정하지 않고도 완전한 Layer 2 실행 환경을 구축할 수 있습니다
...
python examples/bridge_relay_modes.py   # 5가지 릴레이 모드 비교
```
...
| `zk/` | 6 | 1,844 | Groth16 circuit 빌더, prover, verifier, EVM 검증자, snarkjs 호환 |
| `bridge/` | 6 | 1,056 | CrossDomainMessenger, BridgeWatcher, BridgeEnvironment, 플러거블 릴레이 핸들러, force inclusion, escape hatch |
| `rpc/` | 6 | 1,838 | JSON-RPC 서버, eth API, Engine API, ZK API |
...
| `test_bridge_proof_relay.py` | 470 | 28 | Proof 기반 릴레이 핸들러 (EVM, Merkle, ZK, TinyDB, Direct) |
...
네. py-ethclient에는 내장된 실행 훅 시스템(`ExecutionHook`)과 L1↔L2 General State Bridge(`CrossDomainMessenger`)가 있습니다. 훅 시스템으로 EVM 동작을 커스터마이징하고, 브릿지로 크로스 도메인 메시징 + force inclusion(검열 저항) + escape hatch(가치 복구)를 구현할 수 있습니다. 코어 코드 수정 없이 L2 개발이 가능합니다.
