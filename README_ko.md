...
- **1,031개 테스트** — RLP부터 ZK 증명, L2 롤업, 통합 테스트까지 전 프로토콜 레이어를 커버하는 포괄적 테스트 스위트
...
롤업 프레임워크의 실전 활용법을 보여주는 9개 앱 예제:

| 예제 | 설명 | 실행 |
|---|---|---|
| **ERC20 토큰** | Mint, transfer, burn + 관리자 권한 | `python examples/apps/l2_token.py` |
| **네임 서비스** | ENS 스타일 도메인 등록, 수정, 이전 | `python examples/apps/l2_nameservice.py` |
| **투표/거버넌스** | 제안 생성, 가중 투표, 정족수 기반 확정 | `python examples/apps/l2_voting.py` |
| **가위바위보** | Commit-reveal 방식 게임 + 베팅/정산 | `python examples/apps/l2_rps_game.py` |
| **DEX (AMM)** | Constant product AMM, LP 토큰, 0.3% 수수료, 슬리피지 체크 | `python examples/apps/l2_dex.py` |
| **NFT 마켓플레이스** | Mint, 리스팅, 구매, 이전 + 5% 크리에이터 로열티 | `python examples/apps/l2_nft_marketplace.py` |
| **멀티시그 지갑** | N-of-M 승인, 제안/승인/실행 워크플로우 | `python examples/apps/l2_multisig.py` |
| **에스크로** | 3자 에스크로 + 상태 머신 (Created→Funded→Released/Refunded) | `python examples/apps/l2_escrow.py` |
| **예측 시장** | 마켓 생성, 베팅, 오라클 해결, 비례 배분 | `python examples/apps/l2_prediction_market.py` |

### 인프라 예제

프로덕션 기능을 시연하는 10개 인프라 예제:

| 예제 | 설명 | 실행 |
|---|---|---|
| **RPC 서버** | JSON-RPC 서버 + APIKey/RateLimit/RequestSize 미들웨어 | `python examples/infra/l2_rpc_server.py` |
| **영속 상태** | LMDB 영속화 + WAL 기반 크래시 복구 | `python examples/infra/l2_persistent_state.py` |
| **DA 프로바이더** | Local vs Calldata (EIP-1559) vs Blob (EIP-4844) 비교 | `python examples/infra/l2_da_providers.py` |
| **검열 저항** | Force inclusion + escape hatch (시퀀서 검열 시) | `python examples/infra/l2_censorship_resistance.py` |
| **멀티 배치 운영** | 15-batch 루프 + prove/submit 분리 | `python examples/infra/l2_multi_batch_ops.py` |
| **Config 풀스택** | L2Config 기반 백엔드 자동 와이어링 + NativeProver fallback | `python examples/infra/l2_config_fullstack.py` |
| **상태 브릿지** | L1↔L2 general state bridge end-to-end | `python examples/infra/general_state_bridge.py` |
| **릴레이 모드** | EVM, Merkle, ZK proof, TinyDB, Direct 릴레이 비교 | `python examples/infra/bridge_relay_modes.py` |
| **ZK 노트북** | Groth16 circuit → setup → prove → verify → EVM 데모 | `python examples/infra/zk_notebook_demo.py` |
| **ZK 노트 정산** | ZK proof 기반 노트 정산 | `python examples/infra/zk_note_settle.py` |
...
| **Poseidon Hash** | ZK-friendly 해시 (~240 R1CS constraints vs keccak256의 ~150,000), circuit 인코딩으로 proof 내 Merkle tree 지원 |
...
pytest tests/test_l2_framework_hardening.py # 프레임워크 강화: config 검증, 스레드 안전성, 생존성, 재시도, L1 최종성
...
pytest tests/test_poseidon.py            # Poseidon 해시, circuit, trie, L2 통합
...
이 프로젝트에는 도메인별 전문 가이드를 제공하는 7개의 [Claude Code](https://claude.com/claude-code) 스킬이 포함되어 있습니다. Claude Code에서 `/<스킬명>`으로 호출할 수 있습니다.

| 스킬 | 명령어 | 설명 |
|------|--------|------|
| L2 Rollup | `/l2-rollup <앱 이름이나 유스케이스>` | App-Specific ZK Rollup 생성 — STF 정의, PythonRuntime, Rollup 라이프사이클 |
| ZK Circuit | `/zk-circuit <회로나 검증 대상>` | ZK 회로 빌드 & Groth16 증명 — setup, prove, verify, EVM on-chain 검증 |
| Bridge | `/bridge <유스케이스나 방향>` | L1↔L2 브릿지 구축 — CrossDomainMessenger, force inclusion, escape hatch |
| Sepolia Deploy | `/sepolia-deploy <앱이나 컨트랙트>` | Sepolia 테스트넷 배포 — EthL1Backend, EIP-1559 tx, on-chain 검증 |
| Test | `/test <모듈이나 기능>` | 테스트 실행 & 작성 — pytest 명령, 패턴, coverage |
| L1 Node | `/l1-node <작업이나 질문>` | L1 Ethereum 노드 — EVM, eth/68, snap/1, Engine API, P2P |
| Debug P2P | `/debug-p2p <에러 메시지나 증상>` | P2P 네트워킹 디버깅 — RLPx, devp2p, 동기화 문제 진단 |
...
│   ├── hash.py                      # Poseidon 해시 (ZK-friendly, BN128 스칼라 필드)
...
│   ├── poseidon_circuit.py          # Poseidon 해시 R1CS 인코딩 (~240 constraints)
...
└── examples/
    ├── apps/                            # L2 애플리케이션 STF 예제
    │   ├── l2_token.py                  # ERC20 토큰 (mint/transfer/burn)
    │   ├── l2_nameservice.py            # ENS 스타일 네임 서비스
    │   ├── l2_voting.py                 # 거버넌스 (제안/투표/확정)
    │   ├── l2_rps_game.py               # Commit-reveal 가위바위보
    │   ├── l2_dex.py                    # AMM DEX (constant product, LP 토큰)
    │   ├── l2_nft_marketplace.py        # NFT 마켓플레이스 (로열티)
    │   ├── l2_multisig.py               # N-of-M 멀티시그 지갑
    │   ├── l2_escrow.py                 # 3자 에스크로 서비스
    │   └── l2_prediction_market.py      # 예측 시장 (오라클)
    ├── infra/                           # 프로덕션 인프라 예제
    │   ├── l2_rpc_server.py             # RPC 서버 + 미들웨어
    │   ├── l2_persistent_state.py       # LMDB 영속화 + 크래시 복구
    │   ├── l2_da_providers.py           # DA 레이어 비교 (Local/Calldata/Blob)
    │   ├── l2_censorship_resistance.py  # Force inclusion + escape hatch
    │   ├── l2_multi_batch_ops.py        # 멀티 배치 운영 루프
    │   ├── l2_config_fullstack.py       # L2Config 풀스택 와이어링
    │   ├── general_state_bridge.py      # L1↔L2 브릿지 end-to-end
    │   ├── bridge_relay_modes.py        # 릴레이 모드 비교
    │   ├── zk_notebook_demo.py          # ZK 툴킷 end-to-end
    │   └── zk_note_settle.py            # ZK 노트 정산
    ├── l2_sepolia_hello.py              # Sepolia 라이브 배포
    └── l2_sepolia_all.py                # Sepolia 4-앱 배치 배포
```
...
| 모듈 | 파일 | LOC | 설명 |
|---|---:|---:|---|
| `l2/` | 24 | 3,291 | 앱 특화 ZK 롤업: STF, 시퀀서, 프로버, L1 백엔드, 롤업 오케스트레이터, RPC, CLI, 프로덕션 DA (S3/Calldata/Blob), 네이티브 프로버, LMDB 상태, 미들웨어 |
| `common/` | 7 | 2,444 | RLP, types, trie (+ 증명), crypto, Poseidon 해시, config |
...
| `zk/` | 8 | 2,062 | Groth16 circuit 빌더, prover, verifier, EVM 검증자, Poseidon circuit, snarkjs 호환, R1CS export |
...
| `test_l2_framework_hardening.py` | 629 | 44 | 프레임워크 강화: config 검증, 스레드 안전성, 생존성, LMDB 리사이징, WAL 확장, 재시도, L1 최종성 |
...
| `test_poseidon.py` | 371 | 44 | Poseidon 해시, circuit (R1CS), trie 통합, L2 config/state/rollup |
