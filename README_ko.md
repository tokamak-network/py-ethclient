...
- **785개 테스트** — RLP부터 ZK 증명, L2 롤업, 통합 테스트까지 전 프로토콜 레이어를 커버하는 포괄적 테스트 스위트
...
| `ethclient l2 start --config <path>` | L2 롤업 노드 시작 |
| `ethclient l2 prove --batch <n>` | 배치에 대한 ZK 증명 생성 |
| `ethclient l2 submit --batch <n>` | 증명된 배치를 L1에 제출 |
...
| 모듈 | 파일 | LOC | 설명 |
|---|---:|---:|---|
| `l2/` | 14 | 1,276 | 앱 특화 ZK 롤업: STF, 시퀀서, 프로버, L1 백엔드, 롤업 오케스트레이터, RPC, CLI |
...
|---|---:|---:|---|
| `test_l2_types.py` | 139 | 17 | L2 tx 타입, 인코딩/디코딩, 상태 스냅샷 |
| `test_l2_da.py` | 56 | 8 | 데이터 가용성 프로바이더 |
| `test_l2_runtime.py` | 99 | 9 | Python STF 런타임 래퍼 |
| `test_l2_sequencer.py` | 162 | 10 | 시퀀서, mempool, 배치 조립, 자동 봉인 |
| `test_l2_prover.py` | 91 | 10 | Groth16 증명 백엔드, 필드 절삭 |
| `test_l2_l1.py` | 86 | 6 | L1 백엔드, 증명 검증, 배치 추적 |
| `test_l2_integration.py` | 229 | 12 | 전체 사이클: counter STF, 잔액 이체, 멀티 배치 |
...
- **L2 CLI** — `l2 start`, `l2 prove`, `l2 submit` 서브커맨드는 스캐폴딩만 되어 있음 (Python API를 직접 사용)
- **L2 RPC** — `register_l2_api()`가 존재하지만 CLI에서 자동 시작되지 않음
