...
- **802개 테스트** — RLP부터 ZK 증명, L2 롤업, 통합 테스트까지 전 프로토콜 레이어를 커버하는 포괄적 테스트 스위트
...
| 모듈 | 파일 | LOC | 설명 |
|---|---:|---:|---|
| `l2/` | 14 | 1,415 | 앱 특화 ZK 롤업: STF, 시퀀서, 프로버, L1 백엔드, 롤업 오케스트레이터, RPC, CLI |
...
| `test_l2_sequencer.py` | 174 | 11 | 시퀀서, mempool, 배치 조립, 자동 봉인, nonce gap |
| `test_l2_prover.py` | 134 | 10 | Groth16 증명 백엔드, 필드 절삭 |
| `test_l2_l1.py` | 86 | 6 | L1 백엔드, 증명 검증, 배치 추적 |
| `test_l2_rpc.py` | 119 | 12 | L2 RPC API (l2_* 메서드) |
